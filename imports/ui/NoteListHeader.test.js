...
if(Meteor.isClient) {
  describe('NoteListHeader', function() {
    ...
    it('should call meteorCall on click', function() {
      const wrapper = mount(<NoteListHeader meteorCall={ meteorCall } Session={ Sesstion } />);

      wrapper.find('button').simulate('click');
      meteorCall.calls[0].arguments[1](undefined, notes[0]._id);

      expect(meteorCall.calls[0].arguments[0]).toBe('notes.insert');
      expect(Sesstion.set).toHaveBeenCalledWith('selectedNoteId', notes[0]._id);
    });
    ...
    it('should not set session for failed insert', function() {
      ...
      expect(meteorCall.calls[0].arguments[0]).toBe('notes.insert');
      expect(Sesstion.set).toNotHaveBeenCalled();
    });
  });
}